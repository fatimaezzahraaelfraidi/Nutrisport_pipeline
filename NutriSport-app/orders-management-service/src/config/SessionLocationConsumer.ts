...
export class SessionLocationConsumer {
  ...
  private consumerLocation = this.kafka.consumer({ groupId: 'sessionLocation1-consumer' });
  ...
  async consumeLocationSessionAdd(): Promise<void> {
    ...
    await this.consumerLocation.subscribe({ topic: 'sessionLocation-topic2', fromBeginning: true });
