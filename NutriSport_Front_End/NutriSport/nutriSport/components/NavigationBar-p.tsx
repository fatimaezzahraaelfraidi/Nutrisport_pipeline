import { faHome, faBell, faGear, IconDefinition, faPhoneSquare, faBullhorn, faMap } from '@fortawesome/free-solid-svg-icons';
...
const NavigationBar = ({ screenWidth }: { screenWidth: number }) => {
  ...
  return (
    <View style={{ alignItems: 'center', position: 'absolute', bottom: 10, transform: [{ translateX: screenWidth * 0.05 }] }}>
      ...
      >
        {renderButton(faBullhorn, ' Offers', ' Offers', 'MyOffers')}
        {renderButton(faHome, 'Home', 'Home', 'Demands')}
        {renderButton(faMap, 'Map', 'Map', 'DemandsMap')}
        {renderButton(faBell, 'Orders', 'Orders', 'MyOrders')}
